...
void setupVertices(void)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(cubeTextureCoord), cubeTextureCoord, GL_STATIC_DRAW); // 将纹理坐标数据传递到 VBO
    glVertexAttribPointer(3, 2, GL_FLOAT, false, 0, (void *)0); // 设置顶点属性指针：坐标
    glEnableVertexAttribArray(3);   

    cubeTexture= util.loadTexture("./texture/alien.jpg"); // 加载纹理
    if (cubeTexture <= 0)
    {
        cout << "Error loading texture" << endl;
        // exit(1);
...
void display(GLFWwindow *window, double currentTime)
{
    ...
    * glm::rotate(glm::mat4(1.0f), (float)currentTime*0.3f, glm::vec3(1.0f, 0.0f, 0.0f)) ;
...
// draw skybox
    ...
    glVertexAttribPointer(1, 2, GL_FLOAT, false, 0, (void *)0); // 设置顶点属性指针：纹理坐标
    glEnableVertexAttribArray(1);                               // 启用顶点属性
