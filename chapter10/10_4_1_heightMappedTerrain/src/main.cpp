...
GLuint heightMap; // height map texture
GLuint texture;

ImportedModel ground("./texture/grid.obj");
...
void setupVertices_sphere(void) {
	...
	for (int i = 0; i < mySphere.getNumVertices(); i++) {
		...
		tangentsValues.push_back(tangents[i].x);
		tangentsValues.push_back(tangents[i].y);
		tangentsValues.push_back(tangents[i].z);
	}
	glGenVertexArrays(1, vao);
	glBindVertexArray(vao[0]);
	glGenBuffers(numVBOs, vbo);
...
void init(GLFWwindow* window) {
	...
	texture = Utils::loadTexture("./texture/heightTexture.jpg");
	heightMap = Utils::loadTexture("./texture/height.jpg");
}
